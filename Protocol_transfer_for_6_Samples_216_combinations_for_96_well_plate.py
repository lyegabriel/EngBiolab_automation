...
def run(protocol: protocol_api.ProtocolContext):
    ...
    n = 6 #refers to the number of variations of the same sample (i.e A1.A2...)
    i = 1
    count = 0
...
#     For D2 (Backbone+pLac)
        ...
        for r in range(n):
            if i > 4:
                if i % 2 != 0:
                    ...
                    p10.distribute(2, Falcon["A"+ str(i)], plate3.rows()[r][n:n*2], 
                             new_tip = 'never')
            elif i > 2:
                if i % 2 != 0:
    ...
    while i<(n+1):
        ...
        p10.distribute(2, Falcon['B' + str(i)], plate3.rows()[count][0:n], 
                              new_tip = 'always')
        p10.distribute(2, Falcon['B' + str(i)], plate3.rows()[count][n:n*2], 
                              new_tip = 'always')
        i += 1
        count += 1
   ...
   #For C     
    while i<(n+1):
        ...
        p10.transfer(2, Falcon['C' + str(i)], plate3.columns()[count][0:n], 
                              new_tip = 'always', blow_out = True)
        p10.transfer(2, Falcon['C' + str(i)], plate3.columns()[count+n][0:n], 
                              new_tip = 'always', blow_out = True)
        i += 1
        count +=1
